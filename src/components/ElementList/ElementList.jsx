...
function ElementList({ index, errorMessage, type }) {
  ...
  const classSelected = useSelector(
    (fullState) => fullState.allBuilds.character.classes
  );
  const heroSelected = useSelector(
    (fullState) => fullState.allBuilds.character.hero
  );
  return (
    <>
        ...
        {type === 'spell' ? (
          <>
            {elementList[type]
              .filter(
                (classes) =>
                  (classes.classes === `${classSelected}` &&
                    classes.hero === '') ||
                  classes.hero === `${heroSelected}`
              )
              .map((spell) => (
                <SingleSpell
                  key={spell.value}
