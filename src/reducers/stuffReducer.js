...
export const initialStateStuff = {
  ring: [{ value: '' }, { value: '' }, { value: '' }, { value: '' }],
  cuff: [{ value: '' }],
  companion: [{ value: '' }, { value: '' }, { value: '' }, { value: '' }],
};
...
const stuffReducer = (state = initialStateStuff, action = {}) => {
  ...
  switch (action.type) {
    case SET_ITEM: {
      return {
        ...state,
        [action.payload.type]: state[action.payload.type].map((item, i) =>
    ...
    case DELETE_ITEM: {
      return {
        ...state,
        [action.payload.type]: state[action.payload.type].map((item, i) =>
          i === action.payload.index ? { ...item, value: '' } : item
        ),
      };
    }
    default:
